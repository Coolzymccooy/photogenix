 ...
 * -----------------------------------------------------------------
 * LOCAL: Run `npm run dev`. Ensure the API_KEY environment variable is present in your build tool.
 * PRODUCTION: Build via `npm run build`. Deploy the `dist` folder to Netlify, Vercel, or S3.
 * -----------------------------------------------------------------
 */
...
import { transformImage, developRaw, transformVideoMock, superProcess, combineImages, healSpot, applyWatermark } from './services/geminiService';
import { AuthService, UserProfile } from './services/authService';
import { StorageService } from './services/storageService';
import { Analytics } from './services/analyticsService';
import { Scan, Cpu, Timer, PackageOpen, Layers, HelpCircle, Sparkles, Binary } from 'lucide-react';
import { ToolType, ProjectItem, HistoryItem, MediaType, CreationType } from './types';
...
const App: React.FC = () => {
  ...
  const handleAddFile = async (type: MediaType) => {
    ...
    input.type = 'file';
    input.accept = type === 'image' ? 'image/*, .dng, .cr2, .nef, .arw' : 'video/*';
    input.multiple = true;
    input.onchange = async (e) => {
      ...
      for (const file of files) {
        ...
        newItems.push({
          id, mediaType: type, originalUrl: URL.createObjectURL(file), processedUrl: null,
          status: isRaw ? 'developing' : 'idle', history: [], checkpoints: [], isRaw, fileExtension: ext,
        });
      }
      ...
      // Trigger auto-development for RAW files via the Gemini Bridge
      newItems.filter(i => i.isRaw).forEach(async (rawItem) => {
        try {
          const processedUrl = await developRaw(rawItem.originalUrl, rawItem.fileExtension || 'dng');
          const resp = await fetch(processedUrl);
          const blob = await resp.blob();
          await StorageService.persistBlob(rawItem.id, 'proc', blob);
          
          const historyItem: HistoryItem = { id: Date.now().toString(), url: processedUrl, prompt: 'RAW_DEVELOPMENT', timestamp: Date.now(), toolLabel: 'NEURAL_DEVELOP' };
      ...
      const results = await Promise.all(targets.map(async (target) => {
        ...
        try {
          ...
          if (activeTool === 'watermark' || activeTool === 'logo-gen') {
             ...
             resultUrl = await transformImage(sourceUrl, `Re-crop and intelligently extend this image to a ${ratio} aspect ratio. Fill gaps with context-aware textures. IMAGE ONLY.`, ratio);
          } else if (target.mediaType === 'image') {
            resultUrl = (activeTool === 'super-engine' || activeTool === 'neural-beauty') ? await superProcess(sourceUrl) : await transformImage(sourceUrl, instruction);
          } else {
        ...
        } catch (e) { return { id: target.id, error: true }; }
      }));

      setItems(prev => prev.map(item => {
        const res = results.find(r => r.id === item.id);
        if (!res || res.error) return item;
        return { 
          ...item, processedUrl: res.resultUrl, status: 'done' as const, lastTool: toolLabel,
          history: [...item.history, res.historyItem], checkpoints: [...item.checkpoints, res.historyItem],
  ...
  const RenderWorkspace = () => {
    ...
    if (!currentItem) return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
         <div className="p-20 border border-line bg-surface/10 rounded-sm flex flex-col items-center group cursor-pointer hover:bg-surface/30 transition-all border-dashed" onClick={() => handleAddFile('image')}>
  ...
  return (
    <div className="flex flex-col h-screen bg-base overflow-hidden">
      {viewState === 'landing' ? (
        <LandingPage onGetStarted={() => setIsAuthModalOpen(true)} isAuthenticated={!!user} onReturnToApp={() => setViewState('app')} />
      ) : (
        <div className="flex flex-1 overflow-hidden">
          <ToolsPanel 
            onProcess={handleProcess} isProcessing={isProcessing} activeTool={activeTool} 
            setActiveTool={setActiveTool} projectCount={items.length} onGoHome={() => setViewState('landing')} 
            activeMediaType={activeMediaType} onSwitchMediaType={setActiveMediaType} 
            rangeValue={spotRadius} onRangeChange={setSpotRadius}
          ...
          <main className="flex-1 flex flex-col min-w-0 bg-[#010101] relative">
            <header className="h-16 bg-surface border-b border-line flex items-center justify-between px-6 shrink-0 z-40">
               <div className="flex gap-2">
                  <Button variant="secondary" size="sm" onClick={() => setIsReviewOpen(true)} disabled={items.length === 0}>REVIEW_SESSION</Button>
                  <Button variant="ghost" size="sm" onClick={() => setIsCreationOpen(true)} icon={<Sparkles size={14} className="text-signal"/>}>CREATOR_STUDIO</Button>
               </div>
               <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => setIsLensOpen(!isLensOpen)} className={`p-2 rounded-sm border transition-all ${isLensOpen ? 'bg-signal text-white' : 'bg-base text-zinc-500 border-line hover:text-white'}`}><Scan size={18} /></button>
                    <button onClick={() => setIsHelpOpen(!isHelpOpen)} className="p-2 rounded-sm border bg-base text-zinc-500 border-line hover:text-white"><HelpCircle size={18} /></button>
                  </div>
               </div>
            </header>
          ...
          <CheckpointPanel checkpoints={currentItem?.checkpoints || []} currentUrl={currentItem?.processedUrl || null} onRestore={(item) => setItems(prev => prev.map(p => p.id === currentItem?.id ? { ...p, processedUrl: item.url } : p))} onRemove={(id) => setItems(prev => prev.map(p => p.id === currentItem?.id ? { ...p, checkpoints: p.checkpoints.filter(c => c.id !== id) } : p))} onReset={() => setItems(prev => prev.map(p => p.id === currentItem?.id ? { ...p, processedUrl: null } : p))} />
        </div>
      )}

      {viewState !== 'landing' && <GalleryStrip items={items} selectedIds={selectedIds} onSelect={handleSelect} onAddImage={() => handleAddFile('image')} onAddVideo={() => handleAddFile('video')} onAddFolder={() => handleAddFile('image')} onRemove={(id) => setItems(prev => prev.filter(i => i.id !== id))} />}
      {isAuthModalOpen && <AuthModal isOpen={isAuthModalOpen} onClose={() => setIsAuthModalOpen(false)} onSuccess={(u) => { setUser(u); setIsAuthModalOpen(false); setViewState('app'); }} />}
